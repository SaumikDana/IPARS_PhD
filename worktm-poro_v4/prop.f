...
C*********************************************************************
      ...
      INCLUDE 'rock.h'
      INCLUDE 'utldat.h'
      REAL*8 RELC1(3000),RELC2(3000),RELC3(3000)
...
C  SATURATIONS FOR SPECIFIC APPLICATIONS
     ...
     & .AND.NKGSG(I).GT.0) THEN
      ...
      ENDIF
    1 CONTINUE
      CALL GETVAL('MODREL ',MODREL,'I4',3000,0,0,0,NDUM,NERR)
      DO 2 I=1,3000
    ...
    2 CONTINUE
      ...
      CALL GETVAL('SWRES ',SWRES,'R8',3000,0,0,0,NDUM,NERR)
      IF(NDUM.GT.0.AND.NERR.GT.0) THEN
         IF(LEVELC) WRITE(NFOUT,*) ' ERROR READING SWRES! '
      ENDIF
      CALL GETVAL('SORES ',SORES,'R8',3000,0,0,0,NDUM,NERR)
      IF(NDUM.GT.0.AND.NERR.GT.0) THEN
         IF(LEVELC) WRITE(NFOUT,*) ' ERROR READING SORES! '
      ENDIF
      CALL GETVAL('SGRES ',SGRES,'R8',3000,0,0,0,NDUM,NERR)
      IF(NDUM.GT.0.AND.NERR.GT.0) THEN
         IF(LEVELC) WRITE(NFOUT,*) ' ERROR READING SGRES! '
      ENDIF
      CALL GETVAL('KRNW0 ',KRNW0,'R8',3000,0,0,0,NDUM,NERR)
      IF(NDUM.GT.0.AND.NERR.GT.0) THEN
         IF(LEVELC) WRITE(NFOUT,*) ' ERROR READING KRNW0! '
      ENDIF
      CALL GETVAL('LAMBDA ',RPLMDA,'R8',3000,0,0,0,NDUM,NERR)
      IF(NDUM.GT.0.AND.NERR.GT.0) THEN
         IF(LEVELC) WRITE(NFOUT,*) ' ERROR READING LAMBDA! '
      ENDIF
      CALL GETVAL('ENTRYPRES ',ENTRYP,'R8',3000,0,0,0,NDUM,NERR)
      IF(NDUM.GT.0.AND.NERR.GT.0) THEN
...
C  INPUT THREE-PHASE MODEL PARAMETERS
    ...
    4 CONTINUE
      ...
      CALL GETVAL('RELCON1 ',RELC1,'R8',3000,0,0,0,NDUM,NERR)
      CALL GETVAL('RELCON2 ',RELC2,'R8',3000,0,0,0,NDUM,NERR)
      CALL GETVAL('RELCON3 ',RELC3,'R8',3000,0,0,0,NDUM,NERR)
...
C  INPUT PROGRAM DEPTHMOD
      ...
      HDEPMOD=.FALSE.
      NAMPROG='DEPTHMOD '
      CALL GETVALS(NAMPROG,BLKBUF,'BT',0,0,0,200000,L,NERR)
