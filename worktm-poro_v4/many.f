...
! bag8 - error if size parameters aren't big enough
      IF (NUMPRC.GT.1024) THEN
        IF (MYPRC.EQ.0)
     &    WRITE(*,'(2(A,I))')'Error in SETPRCS: NUMPRC=',NUMPRC,
     &      'greater than MXPROC=',1024
        CALL KILLPRC(1)
      ENDIF
...
C*********************************************************************
      ...
      INCLUDE 'layout.h'
      INCLUDE 'mpif.h'
      INTEGER IREQ(20),ISTAT(MPI_STATUS_SIZE)
      LOGICAL FLAG
...
C*********************************************************************
      PARAMETER (NDB=100000000/4)
      INCLUDE 'control.h'
!      INCLUDE 'scrat1.h'
      INCLUDE 'layout.h'
      INCLUDE 'mpif.h'
      INTEGER IREQ(20),ISTATS(MPI_STATUS_SIZE,20)
