...
C INPUT FLOW MODEL INITIAL SCALAR DATA
      ...
      MODACT=17
      IF(PEFLOW.EQ.17.OR.MBPOROE) CALL TISDAT (NERR)
              ! SAUMIK,BGANIS
      MODACT=15

C      MODACT=16
C      IF(PEFLOW.EQ.16.OR.MBPOROE) CALL XISDAT (NERR)
             ! SAUMIK,BGANIS
C      MODACT=15

ctm
C      MODACT=18
C      IF(PEFLOW.EQ.18.OR.MBPOROE) CALL HISDAT (NERR)
              ! SAUMIK,BGANIS
C      MODACT=15
ctm
...
C FLOW MODEL GRID ELEMENT ARRAY INPUT
      ...
      MODACT=17
      IF(PEFLOW.EQ.17.OR.MBPOROE) CALL TIADAT (NERR)
              ! SAUMIK,BGANIS
      MODACT=15

C      MODACT=16
C      IF(PEFLOW.EQ.16.OR.MBPOROE) CALL XIADAT (NERR)
             ! SAUMIK,BGANIS
C      MODACT=15

ctm
C      MODACT=18
C      IF(PEFLOW.EQ.18.OR.MBPOROE) CALL HIADAT (NERR)
              ! SAUMIK,BGANIS
C      MODACT=15
ctm
...
C CONVERT AND SET SOME ELASTIC PROPERTIES
      ...
      IF(.NOT.MBPOROE) THEN
         ...
         JEPROP(6) = N_ROCKD
         JEPROP(7) = N_POR
         ! N_POR IS STILL POROSITY, NOT PORE VOLUME
         JEPROP(8) = N_XC
         JEPROP(9) = N_YC
         JEPROP(10) = N_ZC
...
C INPUT FLOW MODEL TRANSIENT DATA
      ...
      MODACT=17
      IF(PEFLOW.EQ.17.OR.MBPOROE) CALL TTDATA(NERR)
              ! SAUMIK,BGANIS
      MODACT=15

C      MODACT=16
C      IF(PEFLOW.EQ.16.OR.MBPOROE) CALL XTDATA(NERR)
             ! SAUMIK,BGANIS
C      MODACT=15

ctm
C      MODACT=18
C      IF(PEFLOW.EQ.18.OR.MBPOROE) CALL HTDATA(NERR)
              ! SAUMIK,BGANIS
C      MODACT=15
ctm
...
! SAUMIK - POROHEX ELEMENTS NOT TALKING TO FLOW
               ...
               ROCKD(I,J,K) = ROCKD(I,J,K) * (1.D0 - POR(I,J,K))
            ENDDO
         ENDDO
      ENDDO

! CALCULATIONS FOR MANDEL (HOMOGENEOUS PROPERTIES)
      IF(MECH_BC_NCASE.EQ.100) THEN
!         V1 = MODUL(IL2,JL2V(KL2),KL2)
!         V2 = POISS(IL2,JL2V(KL2),KL2)
!         V3 = BIOTA(IL2,JL2V(KL2),KL2)
!         VP0 = POR(IL2,JL2V(KL2),KL2)
         V1 = 8.615250734E+05
         V2 = 0.2
         V3 = 0.8
         VP0 = 0.2
         ...
         MANDEL_NU = V2
         SKEMPTON = V3/(VP0*FLCMP/KDRINV + V3 - VP0*(1.D0-V3))
         MANDEL_NUU = (3.D0*V2 + SKEMPTON*(1.D0-2.D0*V2)*V3)/
     &         (3.D0-SKEMPTON*(1.D0-2.D0*V2)*V3)
         MANDEL_MU = FLVIS
         MANDEL_E = V1
      ENDIF
