...
C*********************************************************************
    ...
    1 DEPTH(I-1,J-1,K-1)=DOWN(1,NBLK)*X+DOWN(2,NBLK)*Y+DOWN(3,NBLK)*Z
      ...
      END
C*********************************************************************
      SUBROUTINE TRANC1 (IDIM,JDIM,KDIM,LDIM,IL1,IL2,JL1V,JL2V,KL1,KL2,
...
C      INCLUDE 'msjunk.h'
      ...
      INTEGER I,J,K,IOFF,JOFF,KOFF,MERR,JL1,JL2
      REAL*8 DUM1,DUM2,CVC
...
C*********************************************************************
    ...
    1 CONTINUE
      ...
      IF(.NOT.MBPOROE) THEN
      ! SAUMIK,BGANIS
      ! IF MBPOROE IS TRUE, WE NEED TO AVOID
      ! REPETITION OF ELEMENT COUNT SINCE FMODBLK=MODBLK
         M=FMODBLK(NBLK)
         NEMOD(M)=NEMOD(M)+N
      ENDIF
...
C      INCLUDE 'msjunk.h'
      ...
      INTEGER JL1V(KDIM),JL2V(KDIM),KEYOUT(IDIM,JDIM,KDIM)
      REAL*8 DEPTH(IDIM,JDIM,KDIM),TOPSURF(JDIM,KDIM)

      CALL BLKDIM(NBLK,ID,JD,KD,MERR)
      CALL BLKOFF(NBLK,IOFF,JOFF,KOFF,MERR)

      IF(MYPRC == 0)THEN
        WRITE(61,*)' DEPTH3 IJKDIM,OFF',IDIM,JDIM,KDIM,JOFF,KOFF
        WRITE(61,*)' DEPTH3 I,K ',IL1,IL2,KL1,KL2
        WRITE(61,*)' DEPTH3 XREC,DEPTH',XREC(1,NBLK),DEPTH(IL1,2,KL1)
      ENDIF
C     CALCULATE TOPSURFACE ELEVATION AGAINST XYZ111,
C     I IS DONWDIRECTION
      DO 7 K=KL1,KL2
         DO 8 J=JL1V(K),JL2V(K)
         IF(DEPTH(IL1,J,K).NE.0.0)THEN
C          THERE IS INPUT FOR TOP SURFACE DEPTH
           TOPSURF(J,K) = DEPTH(IL1,J,K)-XREC(1,NBLK)
C          WRITE(*,*)' IN',DEPTH(IL1,J,K),TOPSURF(J,K),J,K
         ELSE
C          THERE IS NO INPUT FOR TOP SURFACE DEPTH
           TOPSURF(J,K) = 0.0
         ENDIF
    8    CONTINUE
    7 CONTINUE
...
C            DO 6 I=1,IDIM
     ...
     &         DOWN(2,NBLK)*(YREC(JG,NBLK)+YREC(JG+1,NBLK))+
     &         DOWN(3,NBLK)*(ZREC(KG,NBLK)+ZREC(KG+1,NBLK)))
C           WRITE(*,*)' IN2',DEPTH(I,J,K),I,J,K
            DEPTH(I,J,K)=TOPSURF(J,K) + DEPTH(I,J,K)
C           WRITE(*,*)' IN3',DEPTH(I,J,K),I,J,K
    6       CONTINUE
         ENDIF
    5    CONTINUE
      ENDIF
