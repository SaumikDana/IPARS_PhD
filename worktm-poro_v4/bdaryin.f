...
c read info about surface regions: volumes first
      ...
      CALL GETVAL('BND_VOL[ft] ',BND_VOL,'R4',8,60,0,0,NDUM,NERR)
...
C       MODACT=5
C       IF(MODELON(5)) CALL HBDINTAB(NTIME,NERR)
C
CGUS BOUNDARY CONDITIONS NOT FUNCTIONAL FOR THESE MODELS
CC       MODACT=19
CC       IF(MODELON(19)) CALL HBDINTAB(NTIME,NERR)
CC       MODACT=18
CC       IF(MODELON(18)) CALL HBDINTAB(NTIME,NERR)
C
C       MODACT=C
C       IF(MODELON(C)) CALL MMDINTAB(NTIME,NERR)
C      MODACT=13
C      IF(MODELON(13)) CALL TBDINTAB(NTIME,NERR)
C
CGUS MFMFE BOUNDARY CONDITIONS NOT FUNCTIONAL
      MODACT=17
      IF(MODELON(17)) CALL TBDINTAB(NTIME,NERR)
C
C       MODACT=14
C       IF(MODELON(14)) CALL TRBDINTAB(NTIME,NERR)
              MODACT=0
...
c
                     CALL UNPACK_MPFAVOLNEW(I,J,N,BND_VOL(3,I),
     &                                         NBND_TYPE(I))
c----------
                 ELSE
