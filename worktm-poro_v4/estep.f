...
C      INCLUDE 'msjunk.h'
      ...
      MODACT = 17
      IF(PEFLOW.EQ.17.OR.MBPOROE) CALL TSTEP1(NERR)
              ! SAUMIK,BGANIS
      MODACT = 15

C      MODACT = 16
C      IF(PEFLOW.EQ.16.OR.MBPOROE) CALL XSTEP1(NERR)
             ! SAUMIK,BGANIS
C      MODACT = 15

ctm
C      MODACT = 18
C      IF(PEFLOW.EQ.18.OR.MBPOROE) CALL HSTEP1(NERR)
              ! SAUMIK,BGANIS
C      MODACT = 15
ctm
      END
...
C      INCLUDE 'msjunk.h'
      ...
      MODACT = 17
      IF(PEFLOW.EQ.17.OR.MBPOROE) CALL TSTEP2(KONVG,NERR)
              ! SAUMIK,BGANIS
      MODACT = 15

C      MODACT = 16
C      IF(PEFLOW.EQ.16.OR.MBPOROE) CALL XSTEP2(KONVG,NERR)
             ! SAUMIK,BGANIS
C      MODACT = 15

ctm
C      MODACT = 18
C      IF(PEFLOW.EQ.18.OR.MBPOROE) CALL HSTEP2(KONVG,NERR)
              ! SAUMIK,BGANIS
C      MODACT = 15
ctm
      END
...
C      INCLUDE 'msjunk.h'
      ...
      MODACT = 17
      IF(PEFLOW.EQ.17.OR.MBPOROE) CALL TSTEP3(KONVG,NERR)
              ! SAUMIK,BGANIS
      MODACT = 15

C      MODACT = 16
C      IF(PEFLOW.EQ.16.OR.MBPOROE) CALL XSTEP3(KONVG,NERR)
             ! SAUMIK,BGANIS
C      MODACT = 15

ctm
C      MODACT = 18
C      IF(PEFLOW.EQ.18.OR.MBPOROE) CALL HSTEP3(KONVG,NERR)
              ! SAUMIK,BGANIS
C      MODACT = 15
ctm
...
C      INCLUDE 'xarydat.h'
     ...
     &         GETPOROSITY,GETPOREVOLUME,GETUPDATEDVAR1,GETUPDATEDVAR2,
...
C --- SAUMIK,BGANIS
     ...
     &         BULKDEN_3PH,EUPDATECR,FLOW_ERR_MB1,FLOW_ERR_MB2,
     &         COMPUTERESID
...
C GET EPV_FLOW (USED IN THE COUPLING CONVERGENCE CRITERION)
C RES. POR. OBTAINED AFTER MECHANICS - RES.POR OBTAINED FROM UPSCALING
C DIVIDED BY RES. POR. OBTAINED AFTER MECHANICS

C UPSCALE RESERVOIR POROSITY (NOT INCREMENTALLY)
...
C UPSCALE PORE PRESSURE (INCREMENTALLY)

         CALL ESETARYR8N(N_RCP,ZERO,1)
         JARG(1) = 3
         JARG(2) = N_PRES
         JARG(3) = N_PRESN3
         JARG(4) = N_RCP
...
C      MODACT = 16
         CALL ESETARYR8N(N_RCP,ZERO,1)
         CALL CALLWORK(FLOW_ERR_MB2,JARG)
         CALL TIMON(38)
         CALL UPDATE(N_RCP,2)
         CALL TIMOFF(38)
         ...
         JARG(2)=N_PRESS
         JARG(3)=N_RCP
         CALL CALLWORK(GETUPDATEDVAR2,JARG)
         CALL TIMON(38)
...
C DOWNSCALE RESERVOIR POROSITY CHANGE
C INCREMENTALLY, NOT INCREMENTALLY, IMPLIES THE SAME
C SINCE POROELASTICITY IS LINEAR
...
C GET UPDATED PORE VOLUME IN FLOW BLOCK
         ...
         JARG(3)=N_EVOL
         JARG(4)=N_RC
         CALL CALLWORK(GETUPDATEDVAR1,JARG)
         CALL TIMON(38)
         CALL UPDATE(N_PV,2)
         CALL TIMOFF(38)
         MODACT=15
...
C      MODACT = 16
         ...
         JARG(1) = 6
         JARG(2) = N_RCV ! CHANGE IN VOL. STRAIN OVER CURRENT COUPLING ITERATION
         JARG(3) = N_RCP ! CHANGE IN PRESSURE OVER CURRENT COUPLING ITERATION
         JARG(4) = N_BIOTAFLOW
         JARG(5) = N_MODULFLOW
         JARG(6) = N_POISSFLOW
         JARG(7) = N_EVOL
         CALL CALLWORK(COMPUTERESID,JARG)
         ! REQD. FOR NUMERICAL CONTRACTION CONSTANT
      CALL SUMIT(1,RESIDP)
      CALL SPREAD8(1,RESIDP)
         IF(MYPRC.EQ.0) THEN
           IF (ONCEONLY) THEN
     ...
     &            ACCESS='append')
           ENDIF ! IF(ONCEONLY)
           IF(GCITER.EQ.1) RESIDO = RESIDP ! TO AVOID DIVISION BY ZERO
           IF(PEKONVG.EQ.1) ! CONVERGED AT CURRENT TIME STEP
     &        WRITE(NPRESOUT,'(0P,I4,E15.8)')GCITER,RESIDP/RESIDO
           CLOSE(NPRESOUT)
           RESIDO = RESIDP ! STORE RESIDP FROM PREVIOUS COUPLING ITERATION
         ENDIF ! IF(MYPRC.EQ.0)
         ENDIF ! IF(NONMATCHING.AND.HETEROGENEOUS)
         MODACT=15
      ELSE
! bag8, djw
         CALL CALLWORK(EUPDATECR,[8,N_MODUL,N_POISS,N_BIOTA,
     &     N_EPV,N_ECR,N_EVOL,N_VSTRAIN,N_VSTRAIN_INIT])
      ENDIF
...
C*********************************************************************
      ...
      IF (ONCEONLY) THEN
        ...
        IMECH(27) = N_TYPEBD(5)
        IMECH(28) = N_TYPEBD(6)
        ! N_DISPBD IS THE DIRICHLET BC VALUE
        IMECH(29) = N_DISPBD(1)
        IMECH(30) = N_DISPBD(2)
        IMECH(31) = N_DISPBD(3)
        IMECH(32) = N_DISPBD(4)
...
C*********************************************************************
C THE CODE BELOW MAY BE USEFUL FOR CRANFIELD-TYPE RUNS
C*********************************************************************

!C UPSCALE RESERVOIR POROSITY INCREMENTALLY
!
!         CALL ESETARYR8N(N_RC,ZERO,1)
!         JARG(1) = 4
!         JARG(2) = N_PV
...
!C         MODACT = 16
!         CALL ESETARYR8N(N_RC,ZERO,1)
!         CALL CALLWORK(FLOW_ERR_MB1,JARG)
!         CALL TIMON(38)
!         CALL UPDATE(N_RC,2)
!         CALL TIMOFF(38)
!
...
!         CALL TIMOFF(38)
!
!C UPSCALE PORE PRESSURE NOT INCREMENTALLY
!
!         COPYARG(2)=N_PRESS
!         COPYARG(6)=N_PRES
!        MODACT=17
