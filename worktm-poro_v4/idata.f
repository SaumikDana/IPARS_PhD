...
C      CALL SBLKIN (NERR)
C      CALL TIMOFF(12)
C      IF (NERR.GT.0) STOP 'Error in DUAL initialization'
...
C  COMPLETE INPUT OF MODEL SPECIFIC INITIAL DATA
      ...
      IF (MODELON(15)) THEN
         CALL EIADAT(NERR)
         IF (NERR.GT.0) STOP 'Error in EIADAT'
         GOTO 9
      ENDIF
...
C      MODACT=13
C      IF (MODELON(13)) CALL TIADAT(NERR)
C      IF (NERR.GT.0) STOP 'Error in TIADAT'

      MODACT=17
      IF (MODELON(17)) CALL TIADAT(NERR)
...
!---saumik
      ...
      HETEROGENEOUS=.FALSE.
      ! SAUMIK
      ! FLAG FOR CMAME PAPER
      CALL GETVAL('HETEROGENEOUS ',HETEROGENEOUS,'FG',0,0,0,0,NDUM,NERR)
      IF(LEVELC) THEN
        WRITE (NFOUT,"('HETEROGENEOUS = ',T55,L10)") HETEROGENEOUS
      ENDIF
...
! bag8 - binary input
     ...
     &  BINARY_INPUTS,0,0,50,NDUM,NERR)
      ...
      WRITE(*,'(a,i2,2a)')'INCLUDE_BINARY(',I,')=',
...
C bag8, djw - special case for driving models that are not flow models
        ...
        IF (MODELON(15)) THEN
           CALL EISDAT(NERR)
           IF ((NERR.GT.0).AND.LEVELC) WRITE(NFOUT,78) 'EISDAT'
           GOTO 1000
        ENDIF
C      MODACT=14
C      IF (MODELON(14)) THEN
...
C       IF ((NERR.GT.0).AND.LEVELC) WRITE(NFOUT,78) 'HISDAT'
C      MODACT=13
C      IF (MODELON(13)) CALL TISDAT(NERR)
C      IF ((NERR.GT.0).AND.LEVELC) WRITE(NFOUT,78) 'TISDAT'
      MODACT=17
      IF (MODELON(17)) CALL TISDAT(NERR)
...
! saumik
      ...
      CHARACTER*50 TABTIT
      INTEGER LOCTA(3000),LENTA(3000),MARG(3)

      INTEGER KERR,NDUM,NERR,KFERR,NDUM1,NDUM2,NDUM3,NDUM4,I,JCPY(3)
...
C  EXTRACT 2 PHASE RELATIVE PERMEABILITY DATA AND FORM TABLES
      ...
      NKOSW(I)=0
    1 LENTA(I)=0
      CALL GETBLK('KOSW ',BLKBUF,200000,3000,LOCTA,LENTA,NERR)
...
cgp      CALL VIS_SETTAB(I,NKOSW(I),'KOSW_',0.D0,1.D0 )
    ...
    2 CONTINUE
      ...
      NKWSW(I)=0
    3 LENTA(I)=0
      CALL GETBLK('KWSW ',BLKBUF,200000,3000,LOCTA,LENTA,NERR)
...
cgp      CALL VIS_SETTAB(I,NKWSW(I),'KWSW_',0.D0,1.D0 )
    ...
    4 CONTINUE
      ...
      NKOSG(I)=0
    5 LENTA(I)=0
      CALL GETBLK('KOSG ',BLKBUF,200000,3000,LOCTA,LENTA,NERR)
...
cgp      CALL VIS_SETTAB(I,NKOSG(I),'KOSG_',0.D0,1.D0 )
    ...
    6 CONTINUE
      ...
      NKGSG(I)=0
    7 LENTA(I)=0
      CALL GETBLK('KGSG ',BLKBUF,200000,3000,LOCTA,LENTA,NERR)
...
C  EXTRACT 2 PHASE CAPILLARY PRESSURE DATA AND FORM TABLES
      ...
      NPCOW(I)=0
    9 LENTA(I)=0
      CALL GETBLK('PCOW ',BLKBUF,200000,3000,LOCTA,LENTA,NERR)
...
cgp     &        SATWC(I)*1.1D0,1.D0 )
   ...
   10 CONTINUE
      ...
      NPCGO(I)=0
   11 LENTA(I)=0
      CALL GETBLK('PCGO ',BLKBUF,200000,3000,LOCTA,LENTA,NERR)
...
C bag8 - CRASH IF GRID IS TOO FINE FOR FRAME SIZE FILE
      IF ((NX(I)+2*ILAY.GT.200).OR.
     &    (NY(I)+2*JLAY.GT.204).OR.
     &    (NZ(I)+2*KLAY.GT.204)) THEN
        WRITE(*,*)'Error: grid too fine on block',I
        WRITE(*,*)'NX,NY,NZ=            ',NX(I),NY(I),NZ(I)
        WRITE(*,*)'MXRECX,MXRECY,MXRECZ=',200,204,204
        WRITE(NFOUT,*)'Error: grid too fine on block',I
        WRITE(NFOUT,*)'NX,NY,NZ=            ',NX(I),NY(I),NZ(I)
        WRITE(NFOUT,*)'MXRECX,MXRECY,MXRECZ=',200,204,204
        STOP 1
      ENDIF
...
!bag8
      EVFEM_HEX=0
      CALL GETVAL('EVFEM_HEX ',EVFEM_HEX,'I4',0,0,0,0,NDUM,NERR)
      IF ((NERR.GT.0).AND.LEVELC) WRITE(NFOUT,77) 'EVFEM_HEX'
      IF (LEVELC) WRITE(NFOUT,*)'EVFEM_HEX=',EVFEM_HEX
      MFMFE_BRICKS=.FALSE.
      CALL GETVAL('MFMFE_BRICKS ',MFMFE_BRICKS,'FG',0,0,0,0,NDUM,NERR)
...
C      ELSE
      ...
      CALL GETVAL('DX[ft] ',DXREC,'R4',200,10,0,0,NDUM1,NERR)
      IF ((NERR.GT.0).AND.LEVELC) WRITE(NFOUT,77) 'DX'
      CALL DEFAULT(EXTDIST)
      CALL GETVAL('DY[ft] ',DYREC,'R4',204,10,0,0,NDUM2,NERR)
      IF ((NERR.GT.0).AND.LEVELC) WRITE(NFOUT,77) 'DY'
      CALL DEFAULT(EXTDIST)
      CALL GETVAL('DZ[ft] ',DZREC,'R4',204,10,0,0,NDUM3,NERR)
...
! bag8
         REF_GRID_SET = .TRUE.
         IF (LEVELC) WRITE(NFOUT,*)'REFERENCE GRID FOLLOWS:'
...
C  CREATE CORNER LOCATION ARRAYS FOR PRIMARY FAULT BLOCKS
      ...
      CALL ALCMPFAGEA1('XC ',2,0,N_XC,EV_PRCBLK,KERR)
      IF (KERR.GT.0) GO TO 13
      CALL ALCMPFAGEA1('YC ',2,0,N_YC,EV_PRCBLK,KERR)
      IF (KERR.GT.0) GO TO 13
      CALL ALCMPFAGEA1('ZC ',2,0,N_ZC,EV_PRCBLK,KERR)
      IF (KERR.GT.0) GO TO 13
...
! bag8
      ELSE  ! MFMFE_BRICKS is .TRUE.
         IF (.NOT.REF_GRID_SET) THEN
           WRITE(NFOUT,*)'ERROR: USING MFMFE_BRICKS WITHOUT DX,DY,DZ'
           NERR=NERR+1
         ELSE
           CALL CALLWORK(SET_MFMFE_BRICKS,[3,N_XC,N_YC,N_ZC])
...
!        GRID ELEMENT ARRAYS FOR XC,YC,ZC
      ...
      CALL GETVAL2(STR,XLOC,'R8',IDIMG,JDIMG,KDIMG,ND4GA,
     &  NVALS,NERR)
      I=ILAY+1
...
C  CREATE CORNER LOCATION ARRAYS (CORNER POINT OPTION) FOR FLOW BLOCK(S)
      ...
      IF (KNDGRD.EQ.3) THEN
      CALL ALCMPFAGEA2('XC ',2,0,N_XC,EV_PRCBLK,KERR)
      IF (KERR.GT.0) GO TO 13
      CALL ALCMPFAGEA2('YC ',2,0,N_YC,EV_PRCBLK,KERR)
      IF (KERR.GT.0) GO TO 13
      CALL ALCMPFAGEA2('ZC ',2,0,N_ZC,EV_PRCBLK,KERR)
      IF (KERR.GT.0) GO TO 13
      ENDIF
...
C  FLAG_TO READIN GRID DEPTH AND CALCULATE FROM TOP SURFACE

      TOPSURF =.FALSE.
      CALL GETVAL('TOPSURF ',TOPSURF,'FG',0,0,0,0,NDUM,NERR)
      IF ((NERR.GT.0).AND.LEVELC) WRITE(NFOUT,77) 'TOPSURF'
!      IF (LEVELC) WRITE(NFOUT,*)'TOPSURF=',TOPSURF
      ...
      ELSEIF (KNDGRD.EQ.3) THEN
         ...
         NARG(4)=N_YC
         NARG(5)=N_ZC
         CALL CALLWORK(DEPTH3,NARG)
         CALL UPDATE(N_DEPTH,2)
      ELSE
...
C  VOLUME - WEIGHTED PROJECTIONS FOR NONMATCHING GRIDS
      ...
      IF(NONMATCHING) THEN ! SAUMIK,BGANIS
! saumik - specify the last overburden layer and the first underburden layer
!          to minimize intersection calculations
         OB_LAYER = 0
         UB_LAYER = NXDIM(1)+1
         CALL GETVAL('OB_LAYER ',OB_LAYER,'I4',0,0,0,0,NDUM,NERR)
         CALL GETVAL('UB_LAYER ',UB_LAYER,'I4',0,0,0,0,NDUM,NERR)
         CALL PROJECTION_OPERATORS(NERR)
         CALL MPI_BARRIER(MPI_COMM_WORLD,NERR)
         IF(MYPRC.EQ.0)THEN
            WRITE(*,*)"!...OPERATORS COMPUTED...!"
         ENDIF
      ENDIF
...
C      INCLUDE 'msjunk.h'
     ...
     &          ,DEPTH3,SET_MFMFE_BRICKS,DEBUG_MFMFE_BRICKS
...
!bag8
      ...
      CALL GETVAL('EVFEM_HEX ',EVFEM_HEX,'I4',0,0,0,0,NDUM,NERR)
      IF ((NERR.GT.0).AND.LEVELC) WRITE(NFOUT,77) 'EVFEM_HEX'
      WRITE(NFOUT,*)'EVFEM_HEX=',EVFEM_HEX
      MFMFE_BRICKS=.FALSE.
      CALL GETVAL('MFMFE_BRICKS ',MFMFE_BRICKS,'FG',0,0,0,0,NDUM,NERR)
...
C  COMPUTE DEPTH ARRAY AND COUNT ELEMENTS

C  FLAG_TO READIN GRID DEPTH AND CALCULATE FROM TOP SURFACE
      TOPSURF =.FALSE.
      CALL GETVAL('TOPSURF ',TOPSURF,'FG',0,0,0,0,NDUM,NERR)
      IF ((NERR.GT.0).AND.LEVELC) WRITE(NFOUT,77) 'TOPSURF'
!      IF (LEVELC) WRITE(NFCOUT,*)'TOPSURF=',TOPSURF
...
C CREATE REAL*8 POROSITY AND VOLUMETRIC STRAIN ARRAY
         ...
         IF(MBPOROE) THEN
         ! SAUMIK,BGANIS
         ! IF MBPOROE IS TRUE, THIS ARRAY IS USED IN UPSCALING THE
         ! UPDATED POROSITIES AFTER FLOW SOLVE
            CALL ALCGEA ('TRUEPOROSITY ',2,0,N_PORTRUE,KERR)
            IF (KERR.GT.0) GO TO 13
         ! IF MBPOROE IS TRUE, THIS ARRAY IS USED IN DOWNSCALING THE
         ! UPDATED VOLUMETRIC STRAIN AFTER MECHANICS SOLVE
            CALL ALCGEA ('VSTRAIN ',2,0,N_VSTRAIN,KERR)
            IF (KERR.GT.0) GO TO 13
            CALL ALCGEA ('VSTRAIN_INIT ',2,0,N_VSTRAIN_INIT,KERR)
            IF (KERR.GT.0) GO TO 13
         ENDIF
      ELSE
...
C  CREATE RELATIVE PORE VOLUME CHANGE ARRAY
      ...
      ! RELATIVE PORE VOLUME CHANGE AFTER MECHANICS SOLVE
        ...
        IF (KERR.GT.0) GO TO 13
        CALL ALCGEA ('RCP ',2,0,N_RCP,KERR)
        IF (KERR.GT.0) GO TO 13
        CALL ALCGEA ('RCV ',2,0,N_RCV,KERR)
        IF (KERR.GT.0) GO TO 13
      ENDIF
...
C  CREATE PERMEABILITY AND TRANSMISSABILITY ARRAYS
      ...
      IF (KERR.GT.0) GO TO 13
! saumik
      CALL ALCGEA ('XPERM8 ',2,0,N_XPERM_R8,KERR)
      IF (KERR.GT.0) GO TO 13
      CALL ALCGEA ('YPERM8 ',2,0,N_YPERM_R8,KERR)
      IF (KERR.GT.0) GO TO 13
      CALL ALCGEA ('ZPERM8 ',2,0,N_ZPERM_R8,KERR)
      IF (KERR.GT.0) GO TO 13
...
C  CREATE MODEL SPECIFIC ARRAYS
      ...
      IF (MODELON(15)) THEN
         CALL EARRAY(KERR)
         IF (KERR.GT.0) STOP 'Errors in EARRAY'
         GOTO 12
      ENDIF
...
C      ENDIF
      ...
      IF (MODELON(17)) THEN
         ...
         CALL MFARRAY(KERR)
         IF (KERR.GT.0) STOP 'Errors in MFARRAY'
      ENDIF
...
C      IF (ADAPTIVITY) THEN
C        CALL CALLWORK(GETFINEDIM,0)
C        ALLOCATE(FINELEV(IDIM_F,JDIM_F,KDIM_F),STAT=KERR)
C        ALLOCATE(FINEARR(IDIM_F,JDIM_F,KDIM_F),STAT=KERR)
C      ENDIF
...
! bag8
CC*********************************************************************
C      SUBROUTINE FREEADAPT()
CC*********************************************************************
C      USE adaptmod
C      IMPLICIT NONE
C      IF (ALLOCATED(FINELEV)) DEALLOCATE(FINELEV)
C      IF (ALLOCATED(FINEARR)) DEALLOCATE(FINEARR)
C      END
