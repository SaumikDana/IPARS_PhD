...
! added the following on 5th may
real(kind=double) gwad1,gwad2,gwad1save,gwad2save,deltim,&
                  gwad1_derivative,gwad2_derivative
! added the above on 5th may
...
!
!Select a stress-like parameter for nondimensionalizing residual norm
!
if (myprc.eq.0) then
   scaling=1.0d0
endif
...
if(NNTIM.NE.0) then
   ...
   do while(.true.)
      ...
      call load
      !
      !setting displacement B.C.to all zero after the first iteration
      !
      ncase=3
      if(no_iter==0) then
        ncase=1
      ...
      !
      !dirichlet bc
      !ncase=1 = Newton iteration 0
      !ncase=2 = elasticity, iter > 0
      !ncase=3 = plasticity, iter > 0
      !
      call bc(ncase,gciter)
      !
...
! bag8
      ...
      ! added the following on 4th may
      !
      !Compute L2 based on active node (for the payzone only)
      !
      L2=0.0d0
      I=0
      do inode=1,node_tl
         ...
         if(ACTIVE_NODE(inode)>0) then
           I=I+1
           L2=L2+RESIDUE1(NODE_ST_DOF(inode))*&
                 RESIDUE1(NODE_ST_DOF(inode))+&
                 RESIDUE1(NODE_ST_DOF(inode)+1)*&
                 RESIDUE1(NODE_ST_DOF(inode)+1)+&
                 RESIDUE1(NODE_ST_DOF(inode)+2)*&
                 RESIDUE1(NODE_ST_DOF(inode)+2)
         end if
      end do
      L2=L2/scaling/scaling
      ...
      gwad1=L2
      if(mech_bc_ncase.eq.100) gwad1=gwad2 ! mandel
      gwad2=gwad2-gwad1
      gwad1_derivative = (gwad1-gwad1save)*deltim
      gwad2_derivative = (gwad2-gwad2save)*deltim
!      write(*,*)"gwad1=",gwad1
!      write(*,*)"gwad2=",gwad2
!      write(*,*)"gwad1_derivative=",gwad1_derivative
!      write(*,*)"gwad2_derivative=",gwad2_derivative
      ! added the above on 4th may
...
!      elseif(EP_SOLVER_FLAG==1) then
!        CALL SUPERLU_POROHEX(IDIM,JDIM,KDIM,LDIM,IL1,IL2,&
!          JL1V,JL2V,KL1,KL2,KEYOUT,NBLK,KEYOUT_CR,GELEI,&
!          GSTIFF,GSTIFF_ROW_INDEX,GSTIFF_COL_INDEX,&
!          ACTIVE_NODE,RESIDUE,TL_LOAD,NODE_DISPLACEMENT,&
!          NODE_ST_DOF,NODE_ST_GDOF,HYPRE_ROWS,OFNODE_DISP,&
!          OFNODE_DISP_TMP,OFNODE_KEYOUT,NNTIM,TL_NON_ZEROS,&
!          NODE_TL,OFNODE_GNUM,OFNODE_GNUM_TMP,OFNODE_L2GID)
      ...
      NODE_DISPLACEMENT=0.0d0
    end do  !N-R iteration at element level converges
...
ELSE
   ...
   do iload=1,NLOADSTEPS
      ...
      call bc(ncase,gciter)
      !
      !Compute L2 based on active node (for the entire domain)
      !
      L2=0.0d0
      I=0
      do inode=1,node_tl
...
SUBROUTINE MODULE_POST_PRCSS(&
        TIME,DELTIM,GCITER,&           ! CURRENT SOLUTION TIME
...
INTEGER LSIZE,ILOWER,IUPPER,&
        LFSIZE,IFLOWER,IFUPPUER,&
        LALLSIZE,LFALLSIZE,LALLELEM,&
        GSIZE,GFSIZE,MYPRC,NUMPRC,NNTIM,GCITER,MODEL_EP
INTEGER NUMFRAC,MXFRAC,MXFRACFACE,NUMFRACFACE(MXFRAC),&
        FRACFACE(4,MXFRACFACE,MXFRAC)
REAL*8  PFN(MXFRACFACE,MXFRAC)
INTEGER KEYOUT_CR(IDIM,JDIM,KDIM),GELEI(IDIM,JDIM,KDIM)
